...
/* ------------------------------------------------------------------ */
/* MONTRE APP-SHELL — pataje ant koumanseKouteAuth() ak 2 fòm            */
/* enskripsyon yo (Kreye Biznis / Enskri ak Envitasyon).                */
/*                                                                       */
/* RÈZON EGZISTANS: enskriNouvoBiznis()/enskriAkEnvitasyon() konekte      */
/* itilizatè a (createUserWithEmailAndPassword) AVAN yo fin ekri         */
/* dokiman itilizate_biznis/{uid} la. Sa deklanche onAuthStateChanged    */
/* pandan ekriti a toujou an kou → koumanseKouteAuth() ta ka konkli      */
/* "pa gen biznis" pou yon kont ki fèk kreye kòrèkteman. Fòm enskripsyon */
/* yo kounye a itilize { uid, bizId } yo deja gen nan men yo (valè retou */
/* fonksyon enskripsyon an) pou rele fonksyon sa a DIRÈKTEMAN — yo pa    */
/* tann listener a ankò pou ka sa a.                                    */
/* ------------------------------------------------------------------ */
function montreAppShell(kontèks) {
...
formEnskri.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Antre dirèkteman nan app-shell la — PA tann koumanseKouteAuth()
    // "dekouvri" bizId la (gade kòmantè montreAppShell() pi wo a).
    montreAppShell({ uid, bizId, non: formData.get("itilizatèNon"), wòl: "Propriétaire" });
  } catch (error) {
    afficheErèAuth(error.message);
  } finally {
    btn.disabled = false;
  ...
  jwennEnvitasyon(bizIdEnvitasyon, kòdEnvitasyon).then((envitasyon) => {
    ...
    formEnskriEnvitasyon.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        montreAppShell({ uid, bizId, non: envitasyon.non, wòl: envitasyon.wol });
      } catch (error) {
        afficheErèAuth(error.message);
        btn.disabled = false;
      }
...
/* ------------------------------------------------------------------ */
/* KOUTE ETA AUTH — kè a: pase ant ekran auth la ak app-shell la          */
/* ------------------------------------------------------------------ */
koumanseKouteAuth((kontèks) => {
  if (!kontèks) {
    BIZ_ID = null;
    ITILIZATÈ_ID = null;
    ITILIZATÈ_NON = null;
