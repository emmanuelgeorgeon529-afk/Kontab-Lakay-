...
import { verifyePèmisyon, getWolAktyèl } from "../security/accessControl.js";
import { anrejistreAksyon } from "../security/audit.js";
import { MODIL, AKSYON } from "../security/permissions.js";
...
async function soumetAcha(formData) {
  ...
  try {
    verifyePèmisyon(MODIL.PAYABLES, AKSYON.CREATE);
    ...
    await anrejistreAksyon(bizIdAktyèl, {
      ...
      itilizatèNon: itilizatèNonAktyèl,
      wol: getWolAktyèl(),
      modil: MODIL.PAYABLES,
      aksyon: "acha_kredi",
      sib: pieceRef,
      enfòSipl: `Founisè: ${founisèNon} — Total: ${montanTotal} HTG`,
    });
...
async function soumetPemanFounisè(formData) {
  ...
  try {
    verifyePèmisyon(MODIL.PAYABLES, AKSYON.EDIT);
    ...
    await anrejistreAksyon(bizIdAktyèl, {
      ...
      wol: getWolAktyèl(),
      modil: MODIL.PAYABLES,
      aksyon: "peman_founisè",
      sib: pieceRef,
      enfòSipl: `Montan: ${montanPeye} HTG (${modePeman}) — Rete: ${nouvoDèt} HTG`,
    });
